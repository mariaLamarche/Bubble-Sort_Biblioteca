BIBLIOTECA - ORDENAR LIBROS POR TÍTULO (BUBBLE SORT)
==========================================================

--- Catálogo Inicial (Desordenado) ---
  - [ID: 1] "Don Quijote de la Mancha" - Miguel de Cervantes
  - [ID: 2] "Cien años de soledad" - Gabriel García Márquez
  - [ID: 3] "La metamorfosis" - Franz Kafka
  - [ID: 4] "El principito" - Antoine de Saint-Exupéry
  - [ID: 5] "Rayuela" - Julio Cortázar
  - [ID: 6] "Ficciones" - Jorge Luis Borges

--- Iniciando Bubble Sort ---

-> Pasada 1:
  - [ID: 2] "Cien años de soledad" - Gabriel García Márquez
  - [ID: 1] "Don Quijote de la Mancha" - Miguel de Cervantes
  - [ID: 4] "El principito" - Antoine de Saint-Exupéry
  - [ID: 3] "La metamorfosis" - Franz Kafka
  - [ID: 6] "Ficciones" - Jorge Luis Borges
  - [ID: 5] "Rayuela" - Julio Cortázar * (Ordenado)

-> Pasada 2:
  - [ID: 2] "Cien años de soledad" - Gabriel García Márquez
  - [ID: 1] "Don Quijote de la Mancha" - Miguel de Cervantes
  - [ID: 4] "El principito" - Antoine de Saint-Exupéry
  - [ID: 6] "Ficciones" - Jorge Luis Borges
  - [ID: 3] "La metamorfosis" - Franz Kafka * (Ordenado)
  - [ID: 5] "Rayuela" - Julio Cortázar * (Ordenado)

-> Pasada 3:
  - [ID: 2] "Cien años de soledad" - Gabriel García Márquez
  - [ID: 1] "Don Quijote de la Mancha" - Miguel de Cervantes
  - [ID: 4] "El principito" - Antoine de Saint-Exupéry
  - [ID: 6] "Ficciones" - Jorge Luis Borges * (Ordenado)
  - [ID: 3] "La metamorfosis" - Franz Kafka * (Ordenado)
  - [ID: 5] "Rayuela" - Julio Cortázar * (Ordenado)
Nota: No se realizaron intercambios en esta pasada. ¡El catálogo ya está ordenado!


  Catálogo Final Ordenado Alfabéticamente
==========================================================
  - [ID: 2] "Cien años de soledad" - Gabriel García Márquez
  - [ID: 1] "Don Quijote de la Mancha" - Miguel de Cervantes
  - [ID: 4] "El principito" - Antoine de Saint-Exupéry
  - [ID: 6] "Ficciones" - Jorge Luis Borges
  - [ID: 3] "La metamorfosis" - Franz Kafka
  - [ID: 5] "Rayuela" - Julio Cortázar
